...
class DuelScreen:
    def __init__(self, screen, gs: GameState):
        ...
        self.gs.log("Click a hand card, then click a zone to place it.")
        self.gs.log("Monsters go to monster zones | Spells/Traps to spell zones")
    ...
    def _layout(self):
        ...
        self.tooltip_rect = pygame.Rect(5,          self.H//2-80, 160, 160)
    ...
    def _slot(self, rect, card, back=False, hl=False, spell_trap=False):
        if hl:
            bg, border = ZH, GO
        ...
        if back:
            ...
            if spell_trap:
                type_lbl = self.fxs.render(card.card_type.value, True, border)
                self.screen.blit(type_lbl, (rect.x+4, rect.y+22))
            elif card.atk is not None:
                self.screen.blit(self.fs.render(str(card.atk), True, GR), (rect.x+4, rect.y+rect.height-20))
        else:
            if spell_trap:
                hint = self.fxs.render("S/T", True, (50,60,50))
                self.screen.blit(hint, (rect.centerx-hint.get_width()//2, rect.centery-hint.get_height()//2))

    def _label(self, text, x, y):
        s = self.fxs.render(text, True, TS)
                ...
                if card.card_type == CardType.MONSTER:
                    for i, z in enumerate(self.pmz):
                        if z.collidepoint(mx, my) and self.gs.player.field_monsters[i] is None:
                            ...
                            self.gs.log(f"Summoned {card.name} in ATK!")
                            self.sel = None
                            break
                    ...
                    for i, z in enumerate(self.psz):
                        if z.collidepoint(mx, my) and self.gs.player.field_spells[i] is None:
                            ...
                            if card.card_type == CardType.SPELL:
                                ...
                                self.gs.log(f"Set a trap face-down.")
                            self.sel = None
                            break
            else:
                self.sel = None
    ...
    def draw(self):
        ...
        for i, z in enumerate(self.omz):
            card = self.gs.opponent.field_monsters[i]
            self._slot(z, card, back=(card is not None))
        for i, z in enumerate(self.osz):
            card = self.gs.opponent.field_spells[i]
            self._slot(z, card, back=(card is not None), spell_trap=True)

        # Player zones
        for i, z in enumerate(self.pmz):
            self._slot(z, self.gs.player.field_monsters[i])
        for i, z in enumerate(self.psz):
            card = self.gs.player.field_spells[i]
            if card and not card.is_face_up:
                self._slot(z, card, back=True, spell_trap=True)
            else:
                self._slot(z, card, spell_trap=True)
        ...
        if self.tooltip:
            card = self.tooltip
            pygame.draw.rect(self.screen, (20,18,40), self.tooltip_rect, border_radius=8)
            pygame.draw.rect(self.screen, ZB,         self.tooltip_rect, width=1, border_radius=8)
            tx, ty = self.tooltip_rect.x+6, self.tooltip_rect.y+6
            ...
            if card.atk is not None:
                self.screen.blit(self.fxs.render(f"ATK {card.atk}  DEF {card.defense}", True, GR), (tx, ty)); ty += 16
            if card.level:
                self.screen.blit(self.fxs.render(f"Level {card.level}  {card.race}", True, TS), (tx, ty)); ty += 16
            words = card.description.split(); line, lines = [], []
            for w in words:
                line.append(w)
                if self.fxs.size(" ".join(line))[0] > self.tooltip_rect.width-12:
        ...
        if self.sel is not None and self.sel < len(self.gs.player.hand):
            card = self.gs.player.hand[self.sel]
            hint = "Click a monster zone to summon" if card.card_type == CardType.MONSTER else "Click a spell/trap zone to set"
            s = self.fxs.render(hint, True, GO)
            self.screen.blit(s, (180, self.H-88))